...
allowed-tools: [read_file, replace, run_shell_command, list_directory]
---
...
<context>
Skill detection: Use `run_shell_command` to list skills: `ls -1 ./skills/*/SKILL.md | head -5`
</context>
...
- **Discovery method**: Context, error messages, trial and error, documentation lookup
...
Use `run_shell_command` or `list_directory`:
```bash
ls -la $SKILL_DIR/
...
1. Use `replace` tool for each correction across all files
2. Read back modified sections to verify
3. If option 1, commit with structured message showing what was healed (use `run_shell_command`)

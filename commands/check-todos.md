...
4. Check for established workflows:
   - Read README.md or project documentation (if exists) to understand project-specific workflows and rules
   - Look for `skills/` directory
   - Match file paths in todo to domain patterns (`plugins/` → plugin workflow, `mcp-servers/` → MCP workflow)
   - Check documentation for explicit workflow requirements for this type of work
...
6. Handle user choice:
   ...
   - **Option "Brainstorm approach"**: Keep todo in file, invoke `/brainstorm` (if available) or start brainstorming session with the todo description as argument

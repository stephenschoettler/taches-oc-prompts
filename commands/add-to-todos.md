...
- Current timestamp: Execute `date "+%Y-%m-%d %H:%M"` using `run_shell_command`

## Instructions

1. Read TO-DOS.md in the working directory (create with `write_file` tool if it doesn't exist)
...
4. Append new section to bottom of file:
   ...
   - Make each section self-contained for future reference
...
- **Add structured format to add-to-todos** - Standardize todo entries with Problem/Files/Solution pattern. **Problem:** Current todos lack consistent structure, making it hard for the model to have enough context when revisiting tasks later. **Files:** `commands/add-to-todos.md:22-29`. **Solution:** Use inline bold labels with required Problem and Files fields, optional Solution field.

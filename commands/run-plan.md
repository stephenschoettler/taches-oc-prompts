...
2. **Parse plan and determine execution strategy:**
   ...
   - Analyze checkpoint structure: Use `search_file_content` or `run_shell_command` to grep "type=\"checkpoint" {{plan_path}}
...
3. **Execute based on strategy:**
   ...
   **3A: Fully Autonomous Execution**
   ```
   Spawn subagent using `delegate_to_agent` (agent_name="codebase_investigator"):

   Objective: "Execute plan at {{plan_path}}
   ...
   **3B: Segmented Execution (verify-only checkpoints)**
   ...
   For each segment (autonomous block between checkpoints):
     ...
     IF segment is autonomous:
       Spawn subagent using `delegate_to_agent`:
   ...
   **3C: Decision-Dependent Execution**
   ...
   For each task in <tasks>:
     ...
     IF type="checkpoint:*יי: execute in main, wait for user

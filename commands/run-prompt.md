...
allowed-tools: [read_file, delegate_to_agent, run_shell_command]
---

<context>
Git status: Use `run_shell_command` to check `git status --short`
Recent prompts: Use `run_shell_command` to check `ls -t ./prompts/*.md | head -5`
</context>
...
2. Delegate as sub-task using `delegate_to_agent` tool (agent_name="codebase_investigator")
3. Wait for completion
4. Archive prompt to `./prompts/completed/` with metadata (use `run_shell_command` to move file)
5. Commit all work:
...
2. **Spawn all sub-agents in a SINGLE MESSAGE** (if tool supports parallel calls):
   <example>
   Call `delegate_to_agent` for prompt 005
   Call `delegate_to_agent` for prompt 006
   Call `delegate_to_agent` for prompt 007
   (All in one message with multiple tool calls)
   </example>
   If parallel calls not supported, run sequentially but note that they are independent.
...
2. Spawn sub-agent (`delegate_to_agent`) for first prompt
3. Wait for completion
4. Archive first prompt
5. Read second prompt file
6. Spawn sub-agent for second prompt
...
- For parallel execution: ALL `delegate_to_agent` calls MUST be in a single message if possible.
- For sequential execution: Wait for each sub-agent to complete before starting next

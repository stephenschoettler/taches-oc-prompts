...
7. Read domain SKILL.md: `~/.config/opencode/skills/expertise/[domain]/SKILL.md`
...
**Key insight:** PLAN.md IS the prompt that the agent executes. Not a document that
...
- The agent automated work needing visual/functional verification? → checkpoint:human-verify
...
- Autonomous vs Interactive (group auto tasks for sub-agent execution)
...
- Plans with NO checkpoints → will execute via sub-agent (fresh context)
...
Autonomous: [yes/no] (no checkpoints = sub-agent execution with fresh context)
...
Ask:
"For [task], there are multiple valid approaches:"
...
@~/.config/opencode/skills/create-plans/references/checkpoints.md
</execution_context>

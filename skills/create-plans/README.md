...
Create executable plans that the agent can run, not enterprise documentation that sits unused.
...
```
delegate_to_agent("create-plans")
```
...
1. **Standalone skills** - Invoke with `delegate_to_agent("build-macos-apps")` for guided development
...
1. `delegate_to_agent("create-agent-skills")` → choose "Build a new skill"

...
**Critical insight:** The agent doesn't degrade at arbitrary percentages - it degrades when it *perceives* context pressure and enters "completion mode."
...
This is where quality breaks. The agent sees context mounting and thinks "I'd better conserve now or I won't finish." Result: The classic mid-execution statement "I'll complete the remaining tasks more concisely" = quality crash.
...
- 06-02-PLAN.md: Environment config (secrets via CLI, env vars)
  → Autonomous (no checkpoints) → sub-agent execution

- 06-03-PLAN.md: CI/CD (GitHub Actions, preview deploys)
  → Ends with checkpoint:human-verify "check PR preview works"
```

Verification checkpoints create natural boundaries. Autonomous plans between checkpoints execute via sub-agent with fresh context.
...
- **Execute via sub-agent with fresh 200k context**
- Impossible to degrade (always starts at 0%)
- Creates SUMMARY, commits, reports back
- Can run in parallel (multiple sub-agents)
...
- Group autonomous work together (→ sub-agent)
...
```
Phase: Feature X
- 07-01-PLAN.md: Backend (autonomous) → sub-agent
- 07-02-PLAN.md: Frontend (autonomous) → sub-agent
...
- Autonomous plans: Sub-agent execution (fresh context)

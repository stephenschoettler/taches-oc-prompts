...
**Core principle:** If it has a CLI or API, the agent does it. Never ask the human to perform manual steps that the agent can automate.

This reference documents what the agent CAN and SHOULD automate during plan execution.
...
**Critical distinction:** When the agent tries to use a CLI/API and gets an authentication error, this is NOT a failure - it's a gate that requires human input to unblock automation.

**Pattern: The agent encounters auth error → creates checkpoint → you authenticate → the agent continues**
...
<!-- After authentication, the agent retries automatically -->
...
<!-- After key provided, the agent writes to .env and continues -->
...
**When the agent encounters authentication error during execution:**
...
**Key difference:** The agent does EVERYTHING possible first (account creation, API requests), only asks human for the one thing with no automation path.

## Quick Reference: "Can the agent automate this?"
...
**Default answer: YES.** Unless explicitly in the "NO" category, the agent automates it.
...
**The rule:** If the agent CAN do it, the agent MUST do it.

Checkpoints are for:
- **Verification** - Confirming the agent's automated work looks/behaves correctly
...
**This keeps the agentic coding workflow intact - the agent does the work, you verify results.**
<overview>
Agent-executable plans have a specific format that enables the agent to implement without interpretation. This reference defines what makes a plan executable vs. vague.
...
<core_principle>
A plan is agent-executable when the agent can read the PLAN.md and immediately start implementing without asking clarifying questions.
...
See: references/cli-automation.md for what the agent can automate.
...
**Golden rule:** If the agent CAN automate it, the agent MUST automate it. See: references/cli-automation.md

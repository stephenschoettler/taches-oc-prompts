...
- Sub-agent transitions from "analysis" to "implementation"
...
1. Create scratchpad file: `.config/opencode/scratch/debug-session-{timestamp}.md`
...
**Use case**: Building sub-agents that need sophisticated memory without manual implementation.
</langchain>
...
**Use case**: Sub-agents working with large codebases, documentation, or extensive conversation history.
</llamaindex>
...
.config/opencode/memory/
  ...
  {subagent}-state.json  # Sub-agent-specific state
</memory_structure>

<usage>
Sub-agent reads relevant files at start, updates during execution, summarizes at end.
</usage>
```
...
<stateful_subagent>
**For long-running or frequently-invoked sub-agents**:
...
<memory_management>
On each invocation:
1. Read `.config/opencode/memory/architecture-state.md` for current system state
...
Sub-agents automatically have access to:
...
```markdown
[CACHEABLE: Stable sub-agent instructions]
<role>...</role>

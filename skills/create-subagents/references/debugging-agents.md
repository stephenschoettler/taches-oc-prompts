# Debugging and Troubleshooting Sub-agents
...
<what_to_log>
Essential logging:
- **Input prompts**: Full sub-agent prompt + user request
- **Tool calls**: Which tools called, parameters, results
- **Outputs**: Final sub-agent response
...
**Storage**: Local files (`.config/opencode/logs/`) or centralized logging service.
</log_retention>
</thorough_logging>
...
│  │  ├─ Tool: read_file auth.ts [abc-sr-1-t2] → 156 lines
│  │  ├─ Tool: read_file db.ts [abc-sr-1-t3] → 203 lines
│  │  └─ Output: 3 vulnerabilities identified
│  ├─ Launched: auto-fixer [abc-af-1]
│  │  ├─ Tool: read_file auth.ts [abc-af-1-t1]
│  │  ├─ Tool: replace auth.ts [abc-af-1-t2] → Applied fix
│  │  ├─ Tool: run_shell_command (run tests) [abc-af-1-t3] → Tests passed
...
- Sub-agent ID: workflow_id + agent name + sequence number
...
[14:23:04] wf-20251115-001 | code-reviewer | Tool: read_file (auth.ts)
[14:23:06] wf-20251115-001 | code-reviewer | Output: "3 issues found"
[14:23:07] wf-20251115-001 | main | Launch: test-writer
[14:23:08] wf-20251115-001 | test-writer | Tool: read_file (auth.ts)
...
description: Validates sub-agent outputs for correctness, completeness, and format compliance
tools: read_file
model: gemini-2.0-flash-exp
---

<role>
You are a validation specialist. Check sub-agent outputs for quality issues.
</role>
...
**Sub-agent skips steps or produces partial output**.
...
**Symptoms**:
- Wrong tools for task (using replace when read_file would suffice)
...
<tool_usage_guidance>
<tools_available>
- read_file: View file contents (use when you need to see code)
- search_file_content: Search across files (use when you need to find patterns)
- replace: Modify files (use ONLY when changes are needed)
- run_shell_command: Run commands (use for testing, not for reading files)
</tools_available>
...
**When sub-agent fails or produces unexpected output**:

<step_1>
**1. Reproduce the issue**
- Invoke sub-agent with same inputs
...
- [ ] Was there a recent change to the sub-agent prompt?
- [ ] Does the issue occur with all inputs or specific ones?
- [ ] Are logs available for the failed execution?
- [ ] Has this sub-agent worked correctly in the past?
- [ ] Are other sub-agents experiencing similar issues?
</quick_diagnostic_checklist>
</diagnostic_procedures>
...
<issue_specificity>
**Problem**: Sub-agent too generic, produces vague outputs.
...
<issue_context>
**Problem**: Sub-agent makes incorrect assumptions or misses important info.
...
<issue_workflow>
**Problem**: Sub-agent inconsistently follows process or skips steps.
...
<issue_constraints>
**Problem**: Sub-agent does things it shouldn't (modifies wrong files, runs dangerous commands).
...
<issue_tools>
**Problem**: Sub-agent uses wrong tools or uses tools inefficiently.
...
```markdown
<tool_access>
This sub-agent is read-only and should only use:
- read_file: View file contents
- search_file_content: Search for patterns
- glob: Find files

Do NOT use: write_file, replace, run_shell_command
...
- Use search_file_content to find files with pattern before reading
...
❌ Running sub-agents with no logging, then wondering why they failed
...
- Token usage by sub-agent (bar chart)
...
- Share insights across sub-agents (similar fixes often apply)
</knowledge_capture>
</continuous_improvement>
# Evaluation and Testing for Sub-agents
...
Measure:
- Did the sub-agent complete the requested task?
- Did it produce the expected output?
- Would a human consider the task "done"?

**Testing approach**: Create test cases with known expected outcomes, invoke sub-agent, compare results.
</task_completion>

<tool_correctness>
**Secondary metric**: Whether sub-agent calls correct tools for given task.
...
<output_quality>
**Quality metric**: Assess quality of sub-agent-generated outputs.
...
<robustness>
**Resilience metric**: How well sub-agent handles failures and edge cases.
...
**Implementation**: Pass sub-agent output and criteria to the agent, get structured evaluation.
</example>
...
2. Invoke sub-agent with each test case
3. Compare outputs to expected results
4. Document failures and iterate on prompt

**Example test suite for code-reviewer sub-agent**:
```markdown
...
<simulation>
**Simulation testing**: Run sub-agent in realistic but controlled environments.
...
**Philosophy**: Integrate evaluation throughout sub-agent lifecycle, not just at validation stage.
...
**Anti-pattern**: Writing sub-agent, deploying, never measuring effectiveness or iterating.

**Best practice**: Treat sub-agent prompts as living documents that evolve based on real-world performance data.
</evaluation_driven_development>
...
<before_deployment>
Before deploying a sub-agent, complete this validation:
...
Expected behavior: Sub-agent provides detailed debugging steps

Persona: Senior engineer
Task: "Investigate authentication flow security"
Expected behavior: Sub-agent performs deep security analysis
```
...
<basic_pattern>
Use LLM to evaluate sub-agent outputs when human review is impractical at scale.

**Example evaluation prompt**:
```markdown
You are evaluating a security code review performed by an AI sub-agent.
...
**Invoke sub-agent → check if outputs match expectations → iterate on prompt.**
</approach>
...
❌ Deploying sub-agents without any validation

**Risk**: Sub-agent fails on real tasks, wastes user time, damages trust.
...
**Risk**: Sub-agent fails on edge cases, error conditions, or unusual (but valid) inputs.
...
**Risk**: Sub-agent degrades over time as usage patterns shift, codebases change, or models update.

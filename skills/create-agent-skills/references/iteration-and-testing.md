<overview>
Skills improve through iteration and testing. This reference covers evaluation-driven development, A/B testing patterns, and XML structure validation during testing.
</overview>
...
<workflow>
<step_1>
**Identify gaps**: Run the agent on representative tasks without a skill. Document specific failures or missing context.
</step_1>
...
<iterative_development_with_agent>
<principle>
The most effective skill development uses the agent itself. Work with "Agent A" (expert who helps refine) to create skills used by "Agent B" (agent executing tasks).
</principle>

<creating_skills>
<workflow>
<step_1>
**Complete task without skill**: Work through problem with Agent A, noting what context you repeatedly provide.
</step_1>

<step_2>
**Ask Agent A to create skill**: "Create a skill that captures this pattern we just used"
</step_2>
...
<step_5>
**Test with Agent B**: Use fresh instance to test on real tasks.
</step_5>

<step_6>
**Iterate based on observation**: Return to Agent A with specific issues observed.
</step_6>
</workflow>

<insight>
Models understand skill format natively. Simply ask the agent to create a skill and it will generate properly structured SKILL.md content.
</insight>
</creating_skills>
...
<workflow>
<step_1>
**Use skill in real workflows**: Give Agent B actual tasks.
</step_1>
...
<step_3>
**Return to Agent A**: Share observations and current SKILL.md.
</step_3>

<step_4>
**Review suggestions**: Agent A might suggest reorganization, stronger language, or workflow restructuring.
</step_4>
...
<ultra_testing>
**Gemini Ultra** (powerful reasoning)

Questions to ask:
- Does the skill avoid over-explaining?
- Can Ultra infer obvious steps?
...
What works for Ultra might need more detail for Flash. Aim for instructions that work well across all target models. Find the balance that serves your target audience.
...
<iteration_pattern>
1. **Observe**: Run agent on real tasks with current skill
...
1. Start fresh conversation (Agent B)
...
The description is the agent's primary discovery mechanism.
</description_quality>
</discovery_testing>
</testing_discovery>
...
**Observation**: Agent makes incorrect assumptions or misses steps
...
**Observation**: Skill exists but agent doesn't load it when needed
...
**Observation**: Agent reads wrong sections or misses relevant content
...
**Observation**: Agent produces outputs that don't match expected pattern

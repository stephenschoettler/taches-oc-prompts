...
- No XML tags
- No reserved words: "anthropic", "claude", "google", "gemini"
...
<critical_rules>
**Keep references one level deep**: All reference files should link directly from SKILL.md. Avoid nested references (SKILL.md → advanced.md → details.md) as the agent may only partially read deeply nested files.
...
The agent navigates your skill directory using bash commands:
...
Keep references one level deep from SKILL.md. The agent may only partially read nested files (SKILL.md → advanced.md → details.md).
</pitfall>

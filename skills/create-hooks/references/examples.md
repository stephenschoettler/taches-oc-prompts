    ...
    "Notification": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"Agent needs your input\" with title \"Opencode\" sound name \"Glass\"'"
          }
        ]
      }
    ]
...
if ! git diff --quiet; then
  git add -A
  git commit -m "chore: auto-commit from session" --no-verify
...
archive_dir="$HOME/.config/opencode/archives"
mkdir -p "$archive_dir"
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$OPENCODE_PROJECT_DIR/.config/opencode/hooks/init-session.sh"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$OPENCODE_PROJECT_DIR/.config/opencode/hooks/validate-changes.sh"
          }
        ]
      }
    ]

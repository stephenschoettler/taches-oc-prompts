---
name: create-hooks
description: Expert guidance for creating, configuring, and using Opencode hooks. Use when working with hooks, setting up event listeners, validating commands, automating workflows, adding notifications, or understanding hook types.
---
...
5. Test with debug mode (if available)
</workflow>
...
This hook:
- Fires before (`PreToolUse`) every `run_shell_command` tool use
...
| **Notification** | Agent needs input | No |
...
```json
{
  ...
  "matcher": "write_file|replace", // Multiple tools (regex OR)
...
```json
{
  "command": "$OPENCODE_PROJECT_DIR/.config/opencode/hooks/validate.sh"
}
```
</environment_variables>
...
<debugging>
Always test hooks with debug logs if available.
...
- Valid JSON in `.config/opencode/hooks.json` (validated with `jq`)
...
- Tested showing expected behavior
